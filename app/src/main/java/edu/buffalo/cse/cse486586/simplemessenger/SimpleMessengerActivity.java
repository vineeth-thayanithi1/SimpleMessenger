...
public class SimpleMessengerActivity extends Activity {
    ...
    private class ServerTask extends AsyncTask<ServerSocket, String, Void> {
        ...
        protected Void doInBackground(ServerSocket... sockets) {
            ...
            try
            {
                while (true) // Server Waits and Accepts Connection forever
                {
                    ...
                    Log.d(TAG, msgRecv);
                    publishProgress(msgRecv); // Message published to UI
                    in.close();
                    outStream.close();
    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
                 ...
                 */
                DataOutputStream outStream = new DataOutputStream(socket.getOutputStream()); // Output Stream of the connection
                outStream.writeUTF(msgToSend); // Message is written to the output stream which seeps through the socket

                DataInputStream in= new DataInputStream(new BufferedInputStream(socket.getInputStream())); // Input Stream to Receive Ack Packet
                String ack = in.readUTF();
                Log.d(TAG, ack);
                if(ack == "ACK") // Streams are Flushes and CLosed. Connection Ends
                {
